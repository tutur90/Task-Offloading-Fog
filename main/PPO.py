...
from policies.a2c_policy import A2CPolicy
...
def run_epoch(env: Env, policy, data: pd.DataFrame, refresh_rate=1, train=True):
    """
    Run one simulation epoch over the provided task data, adapted for the PPO policy.
    
    For each task:
      - Wait until the task's generation time.
      - Obtain the current state and select an action via the PPO policy (which returns additional info).
      - Schedule the task for processing.
      - Once processed, record the next state and compute the reward.
      - Store the transition with extra PPO information (log_prob, value) for training.
    ...
    # Temporary storage for transitions waiting for a next state update.
    stored_transitions = {}
    ...
    for i, task_info in pbar:
        ...
        while True:
            ...
            if env.now >= generated_time:
                ...
                # If there's a previous transition waiting for a next state, update it.
        ...
        # Store current transition with next_state set to None (will be updated later).
        ...
        for task_id, (state, action, log_prob, value, next_state) in list(stored_transitions.items()):
            if task_id in env.logger.task_info:
                ...
                if val[0] == 0:
                    ...
                    reward = -1e6
                # If next_state is still None, use the current state as a placeholder.
        ...
        if (i + 1) % policy.batch_size == 0 and train:
