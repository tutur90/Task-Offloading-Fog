...
class NATE(BaseModel):
    def __init__(self, d_in, d_pos, d_task, d_model=64, mlp_ratio=4, d_ff=None, n_heads=4, n_layers=3, dropout=0.1, qk_norm=True, learnable_qk_norm=True, embed="regular", **kwargs):
        ...
        if embed == "regular":
            ...
            self.nodes_embed = SwiGLU(d_in, d_model * mlp_ratio, d_model)
        else:
    ...
    def _init_encoder(self, d_model, mlp_ratio, d_ff, n_heads, n_layers, dropout, qk_norm, d_task=None):
        ...
        if n_layers == 0:
            # If no transformer layers, use a simple MLP
...
class MLPConditioner(nn.Module):
    """Task modulates node representations via an MLP."""
...
class TNATE(NATE):
    ...
    def _init_encoder(self, d_model, mlp_ratio, d_ff, n_heads, n_layers, dropout, qk_norm, d_task=None):
        ...
        if n_layers == 0:
            # If no transformer layers, use a simple conditioner + MLP
